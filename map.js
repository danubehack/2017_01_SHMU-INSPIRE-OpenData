...
$.ajax({
	...
	view = new ol.View({
				...
				center : ol.proj.transform([19.149167, 48.7325], 'EPSG:4326', 'EPSG:3857'),
				zoom : 8
			});
	map = new ol.Map({
			layers : [
	...
	select_interaction.getFeatures().on("add", function (e) {
		...
		$.get(urlObservations, function (res) {
			...
			$.each(data, function (k, v) {
				...
				modalContent += '<tr><td>'+property+'</td><td><a class="btn btn-default" href="'+dataURL+'" target="_blank">Data</a><button class="btn btn-default" onclick="tableFromAPI(\'' + property + '\',\'' + dataURL + '\');">Table</button><button class="btn btn-default" onclick="plotFromAPI(\'' + property + '\',\'' + dataURL + '\');">Plot</button></td></tr>';
			})
			modalContent += '</table>';
